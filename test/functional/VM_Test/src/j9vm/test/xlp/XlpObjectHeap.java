...
public class XlpObjectHeap implements XlpTestInterface {
    private ArrayList<XlpOption> xlpTestCodeLise = null;
    private ArrayList<Pair<Long, String>> supportedLargePages = null;
    private final String unsupportedLargePagesMsg = "System configuration does not support option '-XX:+UseLargePagesObjectHeap'";
    private final String componentLabel = "UseLargePagesObjectHeap";
    ...
    public ArrayList<Pair<Long, String>> getSupportedLargePages() throws IllegalArgumentException {
        ...
        switch (osArch) {
            case X86:
                supportedPages = new ArrayList<Pair<Long, String>>() {{ add(new Pair(4 * XlpUtil.ONE_KB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); 
                                                                        add(new Pair(2 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); 
                                                                        add(new Pair(4 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); 
                                                                        add(new Pair(7 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); }};
                break;
            case S390X:
                if (osName == OSName.ZOS) {
                    supportedPages = new ArrayList<Pair<Long, String>>() {{ add(new Pair(4 * XlpUtil.ONE_KB, XlpUtil.XLP_PAGE_TYPE_NONPAGEABLE)); 
                                                                            add(new Pair(1 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_PAGEABLE));
                                                                            add(new Pair(1 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_NONPAGEABLE)); 
                                                                            add(new Pair(2 * XlpUtil.ONE_GB, XlpUtil.XLP_PAGE_TYPE_NONPAGEABLE)); }};
                } else if (osName == OSName.LINUX) {
                    supportedPages = new ArrayList<Pair<Long, String>>() {{ add(new Pair(4 * XlpUtil.ONE_KB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); 
                                                                            add(new Pair(1 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); }};
                } else {
            ...
            case PPC:
                if (osName == OSName.AIX) {
                    supportedPages = new ArrayList<Pair<Long, String>>() {{ add(new Pair(4 * XlpUtil.ONE_KB, XlpUtil.XLP_PAGE_TYPE_NOT_USED));
                                                                            add(new Pair(64 * XlpUtil.ONE_KB, XlpUtil.XLP_PAGE_TYPE_NOT_USED));
                                                                            add(new Pair(16 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); }};
                    if (addrMode == AddrMode.BIT64) {
                        supportedPages.add(new Pair(16 * XlpUtil.ONE_GB, XlpUtil.XLP_PAGE_TYPE_NOT_USED));
                    }
                } else if (osName == OSName.LINUX) {
                    supportedPages = new ArrayList<Pair<Long, String>>() {{ add(new Pair(64 * XlpUtil.ONE_KB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); 
                                                                            add(new Pair(2 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); 
                                                                            add(new Pair(16 * XlpUtil.ONE_MB, XlpUtil.XLP_PAGE_TYPE_NOT_USED)); }};
                } else {
                    throw new IllegalArgumentException("Unsupported Operating System");
	...
	public ArrayList<XlpOption> populateXlpTestCases() {
		...
		/* Return Reference if already allocated */
		...
		xlpTestCaseList.add(new XlpOption("-XX:+UseLargePagesObjectHeap", true));
