...
J9::TreeEvaluator::rdWrtbarHelperForFieldWatch(TR::Node *node, TR::CodeGenerator *cg, TR::Register *sideEffectRegister, TR::Register *valueReg)
   ...
   if (symRef->getSymbol()->isStatic())
      ...
      //dataSnippet = new (cg->trHeapMemory()) TR::J9PPCWatchedStaticFieldSnippet (cg, node, owningMethod, bcIndex, fieldAddress, fieldClass);
      dataSnippet = TR::TreeEvaluator::getFieldWatchStaticSnippet(cg, node, owningMethod, bcIndex, fieldAddress, fieldClass);
      }
   else
      {
      //dataSnippet = new (cg->trHeapMemory()) TR::J9PPCWatchedInstanceFieldSnippet(cg, node, owningMethod, bcIndex, isUnresolved ? -1 : symRef->getOffset() - TR::Compiler->om.objectHeaderSizeInBytes());
