...
uint8_t* TR::J9PPCWatchedStaticFieldSnippet::emitSnippetBody()
    {
    printf("Emitting Static Snippet\n");
    uint8_t *cursor = cg()->getBinaryBufferCursor();
    getSnippetLabel()->setCodeLocation(cursor);
    TR::Node *node = getNode();
    printf("Snippet location:%p\n", cursor);
    ...
    */
    ...
    printf("Correctly emitted method data\n");
    ...
    printf("Created an external relocation for Method\n");
    ...
    if (isResolved)
        ...
        if (cg()->comp()->getOption(TR_UseSymbolValidationManager))
            ...
            {
            /* Comment out when AOT is Enabled
        ...
        if(TR::Compiler->target.is64Bit() && getTOCOffset() != PTOC_FULL_INDEX)
            ...
            printf("Set cursor: %p at TOC Offset :%d\n", (uintptrj_t)cursor, getTOCOffset());
            }
