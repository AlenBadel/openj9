 ...
 * If the value is not supported, the system will set the largest available size instead. The user will also be notified 
 ...
 */
static bool
gcProcessXXObjectHeapArguments(J9JavaVM *vm)
{
	PORT_ACCESS_FROM_JAVAVM(vm);
	MM_GCExtensions *extensions = MM_GCExtensions::getExtensions(vm);
	if (vm->largePageSizeRequested == -1) {
...
gcParseSovereignArguments(J9JavaVM *vm)
{
	...
	if (vm->largePageArgIndex > xlpOptionIndex && !gcProcessXXObjectHeapArguments(vm)) {
		goto _error;
	}
