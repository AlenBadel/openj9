 ...
 * If the value is not supported, the system will set the default Large Page size if any. The user will also be notified 
 * of the altered value being used.
 * 
 */
void
verifyLargePageSize(J9JavaVM *vm, UDATA requestedPageSize, UDATA requestedPageFlags, MM_GCExtensions *extensions)
{
...
/**
 * 
 * Obtains and sets the largest page size available on the system. 
 * Returns true if the operation was successful.
 * 
 */
bool
gcSetPreferredPageSize(J9JavaVM *vm, MM_GCExtensions *extensions)
{
 ...
 *  This is compared to the indices of -Xlp, -Xlp<size>, and -Xlp:objectheap:pagesize=<xx>/
 * 
 */
static bool
gcProcessXXLargePageObjectHeapArguments(J9JavaVM *vm)
{
	...
	if (-1 == vm->largePageSizeRequested) {
...
#if	defined(J9ZOS390)
		/* Get the default large page size of pageable (ZOS Only) */
		UDATA defaultLargePageSizePageable = 0;
		UDATA defaultLargePageFlagsPageable = J9PORT_VMEM_PAGE_FLAG_PAGEABLE;

		j9vmem_default_large_page_size_ex(0, &defaultLargePageSizePageable, &defaultLargePageFlagsPageable);
	
		/* Compare which of the two is larger, and use the larger default page file */
		if (defaultLargePageSizePageable > defaultLargePageSize) {
			defaultLargePageSize = defaultLargePageSizePageable;
			defaultLargePageFlags = defaultLargePageFlagsPageable;
		}	
#endif /* defined(J9ZOS390) */
		...
		// Verify Size
		verifyLargePageSize(vm, largePageSizeRequested, largePageFlagRequested, extensions);
	}
...
gcParseXlpOption(J9JavaVM *vm)
{
	...
	if (-1 != xlpIndex) {
		...
		rc = xlpIndex;
		if (!gcSetPreferredPageSize(vm, extensions)) {
	...
	if ((-1 != xlpMemIndex) || (-1 != xlpObjectHeapIndex)) {
		verifyLargePageSize(vm, requestedPageSize, requestedPageFlags, extensions);
	}
...
gcParseSovereignArguments(J9JavaVM *vm)
{
	...
	if ((vm->largePageArgIndex > xlpOptionIndex) && (!gcProcessXXLargePageObjectHeapArguments(vm))) {
		goto _error;
	}
